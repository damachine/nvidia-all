...
# Direct version selector: if a known tag is provided, prefill version/branch/md5 and skip version prompt.
_set_driver_tag() {
  _driver_version="$1"
  _driver_branch="$2"
  _md5sum="${3:-SKIP}"
...
if [ -n "$_driver_version_tag" ] && [ ! -e options ]; then
  case "$_driver_version_tag" in
    "595.44.03") _set_driver_tag "595.44.05"  "vulkandev" "b4479cedbbbc4a8a5e69d57088cdaa24" ;;
    "595.58.03") _set_driver_tag "595.58.03"  "regular"   "8d98a183bf994af0ff19980e0ef430f2" ;;
    "580.142")   _set_driver_tag "580.142"    "regular"   "30a8bebe011ab3d9ff5d42a1279d4f61" ;;
    *) warning "_driver_version_tag='$_driver_version_tag' is unknown. Falling back to interactive driver version prompt." ;;
  esac
fi
...
if [ -z "$_driver_version" ] || [ "$_driver_version" = "latest" ] || [ -z "$_driver_branch" ] && [ ! -e options ]; then
  ...
  if [[ -z $CONDITION ]]; then
    ...
    if [ "$CONDITION" = "2" ]; then
      echo '_driver_version=595.71.05' > options
      ...
      if [ "$CONDITION" = "2" ]; then
        ...
        echo '_driver_version=535.309.01' > options
        echo '_md5sum=1bd545e7531e2f74b7aa039192c72919' >> options
        echo '_driver_branch=regular' >> options
...
if [ "$_libxnvctrl" = "true" ]; then
  if [ "$_driver_branch" = "vulkandev" ]; then
    ...
    _nvsettings_http_status=$(curl -o /dev/null -s -w "%{http_code}" -L "https://github.com/NVIDIA/nvidia-settings/archive/refs/tags/${_driver_version}.tar.gz")
    if [[ "$_nvsettings_http_status" != "200" ]]; then
      warning "libxnvctrl-tkg: nvidia-settings tag ${_driver_version} not found on GitHub (HTTP ${_nvsettings_http_status})."
      warning "Falling back to external 'libxnvctrl'; replacing an installed TKG provider may require manual 'pacman -S libxnvctrl'."
      _libxnvctrl="external"
    else
      if [ "$_nvsettings" != "true" ]; then
...
md5sums=("$_md5sum"
        ...
        'c488acde6cf5bfed42ee969f28b379dc' # nvidia-bsb-dsc-fix.diff
...
_detect_kernels() {
  ...
  if [ -n "$_target_kernel" ]; then
    local _mod_dir _pkgbase_file _ver_file
    for _mod_dir in /usr/lib/modules/*/; do
      _pkgbase_file="${_mod_dir}pkgbase"
      if [ -f "$_pkgbase_file" ] && [ "$(cat "$_pkgbase_file")" = "$_target_kernel" ]; then
        _ver_file="${_mod_dir}build/version"
        [ -f "$_ver_file" ] || _ver_file="${_mod_dir}extramodules/version"
        [ -f "$_ver_file" ] && _candidates+=("$(cat "$_ver_file")")
      fi
    done
  elif [ -n "$_kerneloverride" ]; then
...
build() {
  ...
  # Change to open source module basedir if applicable
  if [ "$_open_source_modules" = "true" ]; then
    cd "${_srcbase}-${pkgver}"
  ...
  for _kernel in "${_kernels[@]}"; do
    ...
    msg2 "Building $_label module for $_kernel (${BUILD_FLAGS[*]})..."
